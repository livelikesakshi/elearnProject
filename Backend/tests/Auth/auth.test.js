...
const UserCycle=async ()=>{
    ...
    new_User = await User.create({name:'sakshi',email:"sakshi@gmail.com",password:'password',
    isverified:true,courses:[], bookmark:[],preferences:[] });

    new_course = await Course.create({ title: "course_test", category:"Machine Learning",
    name:"sakshi",discription:"something",creator:new_User._id});

    access_token = await jwt.sign({email:"sakshi@gmail.com",userId:new_User._id},global.access_token,{
    algorithm: "HS256",
    expiresIn:"10h"
    });                   
...
describe("Authentication" , ()=>{
    ...
    it("Signup", async()=>{
        ...
        .set(header)
        .send({email:"test@gmail.com", name:'sakshi', password:"password"})
        .expect(201)
        .then(res=>{
        ...
        .send({email:"sakshi@gmail.com",password:"password"})
        .expect(401)
        // .then(res=>{
